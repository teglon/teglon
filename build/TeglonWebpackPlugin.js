...
class TeglonWebpackPlugin {
    ...
    isModuleExternal(module) {
        return module.nameForCondition && EXTERNAL_MODULE_PATTERN.test(module.nameForCondition());
    }

    isRequiredByOwnPackage(ownPackageName, requiringModule) {
        return path.basename(this.getPackageRootPath(requiringModule.context)) === ownPackageName;
    }

    getPackageRootPath(modulePath) {
        modulePath = path.resolve(this.cwd, modulePath);

        while (path.basename(path.dirname(modulePath)) !== 'node_modules' && modulePath !== this.cwd) {
    ...
    apply(compiler) {
        ...
        compiler.options.optimization.splitChunks.cacheGroups.vendors = {
            ...
            name: (module) => {
                ...
                let packageManifest = this.packageMetadata.get(packageRootPath).packageManifest;
		...
		compiler.hooks.compilation.tap('TeglonWebpackPlugin', compilation => {
            ...
            compilation.hooks.finishModules.tapPromise('TeglonWebpackPlugin', async (modules) => {
                let externalModules = modules.filter(module => this.isModuleExternal(module));
                ...
                for await (let module of externalModules) {
                    ...
                    let packagePath = this.getPackageRootPath(module.context);

                    if (this.packageMetadata.has(packagePath)) {
                        continue;
                    }

                    let packageManifest = await this.getPackageManifest(module.context);
                    ...
                    if (module.issuer) {
                        ...
                        if (!this.isRequiredByOwnPackage(packageManifest.name, requiringModule)) {
                            this.moduleMetadata.get(module.identifier()).isPublic = true;
                        }

                        // Walk up the issuer path until the issuer no longer part of the same package
                        // as the module itself. Used to get the semver range the module's package satisfies
                        while (this.isRequiredByOwnPackage(packageManifest.name, requiringModule)) {
                            requiringModule = requiringModule.issuer;
                        }
                        if (requiringModule) {
                            requiringPackageManifest = await this.getPackageManifest(requiringModule.context);
                        }
                    }
                    ...
                    this.packageMetadata.set(packagePath, { packageManifest, requiringPackageManifest });
                }
            });
			...
			compilation.hooks.beforeModuleIds.tap(
				...
				(modules) => {
                    for (module of modules) {
                        let packageRoot = this.getPackageRootPath(module.context);
                        
                        if (!this.packageMetadata.has(packageRoot) || !this.moduleMetadata.get(module.identifier()).isPublic) {
                            continue;
                        }
                        
                        let packageManifest = this.packageMetadata.get(packageRoot).packageManifest;
                        let packageId = `${packageManifest.name}@${packageManifest.version}`;
                        let moduleId = module.identifier().replace(packageRoot, packageId);
        ...
        // compiler.hooks.done.tap('TeglonWebpackPlugin', (stats, ...args) => {
